...
def load_node_infos_db(dbfile,key,value,typeonly):
    ...
    for row in cur:
        ...
        # TODO: The below is a hack to try out different type of representation!
        generalizations = generalize(set_rep);
        set_reps        = [simplify(set_rep) for set_rep in generalizations];
        for set_rep in set_reps:
...
def can_be_merged(i,j,D):
    ...
    if same_slots(gen_rep,spe_rep):
        return generalizes(gen_rep,spe_rep) and set([val for key,val in gen_rep if key=='division'])==set([val for key,val in spe_rep if key=='division']); #True
